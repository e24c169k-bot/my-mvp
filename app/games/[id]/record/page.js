...
function RecordContent() {
  ...
  const [appearedBenchPlayers, setAppearedBenchPlayers] = useState(new Set())
  ...
  const isUsTop = usBattingTurn === 'first'
  const topTeamName = isUsTop ? '自チーム' : (game?.opponent || '相手')
  ...
  const benchPlayers = lineup.filter((l) => {
    ...
    if (!l.is_starter) return !appearedBenchPlayers.has(l.player_id)
    return benchedStarters.has(l.player_id) && !reentryUsed.has(l.player_id)
  })

  function markBenchPlayerAppeared(playerId) {
    if (!playerId) return
    setAppearedBenchPlayers((prev) => {
      if (prev.has(playerId)) return prev
      const next = new Set([...prev, playerId])
      persistGameState({ appearedBenchPlayers: Array.from(next) })
      return next
    })
  }
  ...
  function executeSubstitution(newPlayerId) {
    ...
    if (incomingLineup && !incomingLineup.is_starter) markBenchPlayerAppeared(newPlayerId)
