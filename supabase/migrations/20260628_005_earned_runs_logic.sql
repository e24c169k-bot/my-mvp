alter table stats add column if not exists earned_runs integer default 0;
...
begin
  ...
  -- Earned run decision rule:
  -- Count only scored runner events (to_base='本塁') that are not tagged as
  -- defensive mistakes: エラー・野選 / パスボール.
  update stats s
  set
    earned_runs = (
      ...
      where p.game_id = p_game_id
        ...
        and ra.to_base = '本塁'
        and coalesce(ra.reason, '') not in ('エラー・野選', 'パスボール')
    ),
    era = case
